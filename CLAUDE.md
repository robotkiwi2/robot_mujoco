...
- **에너지는 항상 고통이다 (v0 구현됨)**: 모든 don2 계열 학습(energy=True)은 전력 소비
  (실부품 모델 τ·ω/η+idle, `framework/interoception.py`)를 스텝당 직접 비용으로,
  저SoC(<30%)를 전위 차분 고통으로 보상에 포함하고 [SoC, 전력, 고통]을 관측에 넣는다
  (`framework/affect.py`). 스킬 학습이라도 에너지 절약 압력은 기본 탑재.
  주의: 소비(흐름) 고통은 전위 차분으로 만들면 기울기가 0이 되므로 반드시 직접 비용으로.
