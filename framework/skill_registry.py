...
gen2: v3 발(미니 발판+긴 발가락) 몸에서 학습, 전 스킬 에너지 고통 통일(energy=True).
"""
SKILLS = {
    # ---- 이동 계보 (stand 뿌리) ----
    "stand": dict(
        combo="don2__flat__stand", parent=None, obs_dim=153,
        env_kwargs=dict(mode="stand", energy=True),
    ),
    "walk": dict(
        combo="don2__flat__walk", parent="stand", obs_dim=153,
        env_kwargs=dict(mode="walk", target_speed=0.35, energy=True),
    ),
    "sprint": dict(
        combo="don2__flat__sprint", parent="walk", obs_dim=153,
        env_kwargs=dict(mode="sprint", energy=True),
    ),
    "turn_left": dict(
        combo="don2__flat__turn_left", parent="walk", obs_dim=153,
        env_kwargs=dict(mode="turn_left", target_yaw_rate=0.6, energy=True),
    ),
    "turn_right": dict(
        combo="don2__flat__turn_right", parent="walk", obs_dim=153,
        env_kwargs=dict(mode="turn_right", target_yaw_rate=-0.6, energy=True),
    ),
    # ---- 소근육 계보 (별도 — 자세/역학이 달라 이동 계보에서 이식하지 않음) ----
    "toe_curl": dict(
        combo="don2__flat__toe_curl", parent=None, obs_dim=153,
