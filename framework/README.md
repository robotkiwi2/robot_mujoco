...
| `sensory.py` | 감각피질: 원시신호 → 구조화 지각(percept) | **v0 구현** |
| `association.py` | 연합령: 욕구 평가 → 프로그램 선택/인터럽트 (놀람/휴식/순찰 규칙) | **v0 구현** |
| `program.py` | 운동피질: 행동 프로그램 시퀀서(의도적 스킬 배치) | **v0 구현** |
| `cerebellum.py` | 소뇌: 악보 재생 + 성과 EMA(가소성 입력) | **v0 구현** |
| `brain.py` | 두뇌 통합 루프 (데모: run_don2_brain.py) | **v0 구현** |
| `composer.py` | 합성층: 동결 베이스 혼합가중치+잔차만 학습(전체망 학습 금지 원칙) | 미구현 |
| `plasticity.py` | 가소성 게이트: 성과오차+호르몬 → 스킬 개선 세션(챔피언/도전자 승격) | 미구현 |
